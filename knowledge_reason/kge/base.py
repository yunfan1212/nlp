...
from knowledge_reason.kge.loader import KGTripletLoader
...
class KGEModel(torch.nn.Module):
    ...
    def test(
        ...
        for i in arange:
            h, r, t = head_index[i], rel_type[i], tail_index[i]        #遍历每个样本

            scores = []
            tail_indices = torch.arange(self.num_nodes, device=t.device)     #每一个尾节点
            for ts in tail_indices.split(batch_size):
                scores.append(self(h.expand_as(ts), r.expand_as(ts), ts))      #每个得分

            rank = int((torch.cat(scores).argsort(                                  #降序排列，返回下标列表
                descending=True) == t).nonzero().view(-1))                          #目标值的位置
        ...
        hits_at_k = int(torch.tensor(hits_at_k).sum()) / len(hits_at_k)             #命中的个数
